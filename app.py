...
def main():
    ...
    spcl = st.selectbox('Select number of special request made',[0,1,2,3,4,5])
    ...
    park = (lambda x: 1 if x=='Yes' else 0)(st.text_input('is parking included in the booking',['Yes','No']))
    ...
    wkday_lambda = (lambda x: 0 if x=='Mon' else 1 if x=='Tue' else 2 if x=='Wed' else 3 if x=='Thurs' else 4 if x=='Fri' else 5 if x=='Sat' else 6)
    wkday = wkday_lambda(st.selectbox('What is weekday of arrival',['Mon','Tue','Wed','Thurs','Fri','Sat']))
    
    tran_data = pt.transform([[lt,price]])
    lt_t = tran_data[0][0]
    price_t = tran_data[0][1]
    
    inp_list = [lt_t,mst,spcl,price_t,adults,wkd,wk,park,month,day,wkday]
